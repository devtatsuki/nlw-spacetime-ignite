...
export default function NewMemory() {
  ...
  async function signOut() {
    await SecureStore.deleteItemAsync('nlw_spacetime_token')

    router.push('/')
  }

  async function loadMemories() {
    const token = await SecureStore.getItemAsync('nlw_spacetime_token')
    ...
    >
      <View className="mt-4 flex-row items-center justify-between px-8">
        ...
        <View className="flex-row gap-2">
          ...
          >
            <Icon name="log-out" onPress={signOut} size={16} color="#000000" />
          </TouchableOpacity>
